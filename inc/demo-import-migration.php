 ...
 */
function radiate_demo_import_migration_notice() {
	$demo_imported  = get_option( 'themegrill_demo_importer_activated_id' );
	$notice_dismiss = get_option( 'radiate_demo_import_migration_notice_dismiss' );

	if ( ! $notice_dismiss ) :

		if ( $demo_imported && ( strpos( $demo_imported, 'radiate' ) !== false ) ) :
			?>
			<div class="notice notice-success radiate-notice demo-import-migrate-notice" style="position:relative;">
				<p>
					<?php
					esc_html_e(
				...
				<p>
					...
					>
						<span><?php esc_html_e( 'Fix Imported Demo', 'radiate' ); ?></span>
					</a>

					<a href="<?php echo esc_url( 'https://themegrill.com/contact/' ); ?>" class="btn button-secondary" target="_blank">
						<span><?php esc_html_e( 'Any confusion?', 'radiate' ); ?></span>
					</a>
				</p>
...
add_action( 'admin_notices', 'radiate_demo_import_migration_notice' );


/**
 * Option to dismiss the notice.
 */
function radiate_demo_import_migration_notice_dismiss() {
...
add_action( 'admin_init', 'radiate_demo_import_migration_notice_dismiss' );
 ...
 * @return bool
 */
function radiate_demo_import_migration() {
